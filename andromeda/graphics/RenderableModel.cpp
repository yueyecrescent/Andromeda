...
static Renderable* loadTextures(const char *model_path,Renderable* renderable,aiMaterial *material, aiTextureType type)
{
    ...
    for(unsigned int idx=0;idx<texture_count;++idx)
    {
        ...
        const char* texture_path=str_join(getFileDir(model_path),PATH_SEPARATOR,str.C_Str());
...
static Renderable loadRenderable(const char *model_path,aiMesh *mesh,const aiScene *scene,const char *attrib_order,const char *texture_types)
{
...
static RenderableModel* laodNodeRenderable(const char *model_path,RenderableModel *model,aiNode *node,const aiScene *scene,const char *attrib_order,const char *texture_types)
{
	for(unsigned int idx=0;idx<node->mNumMeshes;++idx)//加载当前节点的全部Mesh
	{
		aiMesh* mesh=scene->mMeshes[node->mMeshes[idx]];
		model->renderable_list.push_back(loadRenderable(model_path,mesh,scene,attrib_order,texture_types));
	}
	for(unsigned int children_idx=0;children_idx<node->mNumChildren;++children_idx)//遍历全部子节点
	{
		return laodNodeRenderable(model_path,model,node->mChildren[children_idx],scene,attrib_order,texture_types);//加载子节点的全部Mesh
	}
	return model;
}

RenderableModel* RenderableModel::loadModel(const char *model_path,const char *attrib_order,const char *texture_types,int post_process)
{
	...
	{
		PRINT_MESSAGE("Loading model failed: ",model_importer.GetErrorString());
		return nullptr;
	}
	RenderableModel* model=new RenderableModel();
	laodNodeRenderable(model_path,model,scene->mRootNode,scene,attrib_order,texture_types);
